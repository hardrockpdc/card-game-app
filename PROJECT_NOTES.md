...
│   ├── EndOfRoundModal.js         (reusable round-end modal — title, message, optional gold `coins` badge, Continue/AdjustBet/Leave buttons; Android Back leaves via onRequestClose; `tableColor` accepted but no longer tints — kept for back-compat)
│   ├── endOfRoundLogic.js         (pure helpers behind EndOfRoundModal — Back-handler resolution, coin-badge guard, double-tap gate; unit-tested)
