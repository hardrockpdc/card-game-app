...
function doSplit(state, playerIndex) {
  ...
  const players = state.players.map((pl, i) =>
    i === playerIndex
      ? { ...pl, hand: [p.hand[0], newCard0], splitHand: [p.hand[1], newCard1], splitStatus: 'playing' }
      : pl
  );
...
function doHit(state, playerIndex) {
  ...
  if (slot === 'split') {
    ...
    updatedPlayer = { ...p, splitHand: newHand, splitStatus: bust ? 'bust' : 'playing' };
  } else {
  ...
  const players = state.players.map((pl, i) => i === playerIndex ? updatedPlayer : pl);
  const updated = { ...state, deck: rest, players };

  const isBust = slot === 'split'
...
function doStand(state, playerIndex) {
  ...
  const updatedPlayer = slot === 'split'
    ? { ...p, splitStatus: 'stand' }
    : { ...p, status: 'stand' };
...
export default function MultiplayerGameScreen({ navigation, route }) {
  ...
  const { phase, currentPlayerIndex, currentHandSlot, players, dealer } = gameState;
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {players.map((player, index) => {
        ...
        const splitHandValue = hasSplit ? calculateHandValue(player.splitHand) : 0;
        ...
        return (
          <View key={String(player.id)} style={[styles.section, isCurrent && styles.sectionActive]}>
              ...
              {!hasSplit && (
                <>
                  ...
                  {phase === 'results' && player.result !== '' && (
                    <Text style={[styles.labelResult, { color: resultColor(player.result) }]}>
            ...
            {hasSplit && (
              <View style={styles.handLabelRow}>
                <Text style={styles.handLabel}>
                  {isCurrent && slot === 'main' ? '▶ ' : ''}Hand 1 = {mainHandValue}
                </Text>
                {phase === 'playing' && player.status === 'bust' && (
                  <Text style={styles.labelBust}>BUST</Text>
                )}
            ...
            <View style={[
              styles.handRow,
              hasSplit && isCurrent && slot === 'main' && styles.activeHandBorder,
            ]}>
            ...
            {hasSplit && (
              <>
                <View style={styles.handLabelRow}>
                  <Text style={styles.handLabel}>
                    {isCurrent && slot === 'split' ? '▶ ' : ''}Hand 2 = {splitHandValue}
                  </Text>
                  ...
                  {phase === 'results' && player.splitResult !== '' && (
                    <Text style={[styles.labelResult, { color: resultColor(player.splitResult) }]}>
