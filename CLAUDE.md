...
**Chat style:** Use the `caveman` skill (full intensity) **and** the `i-have-adhd` skill for every chat response, every session, by default — invoke both at session start without waiting for me to ask. Doesn't apply to persisted content (code, commits, docs, issue notes) — those stay normal prose per those skills' own rules. Off only if I say "stop caveman" / "normal mode". A `SessionStart` hook in `.claude/settings.json` injects this reminder at the start of every session, because relying on the agent to remember it has already failed once — imitating the style from this file is not the same as loading the skills.

Precedence when the two disagree: **caveman governs compression** (word choice, dropped articles, no filler), **`i-have-adhd` governs structure** (action first, numbered steps, state restated every turn, one concrete next action). Structure wins — a clear numbered step beats a shorter fragment. Neither overrides §1 below: when a request is flawed, the objection is the first line, then the action.
