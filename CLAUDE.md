...
- The drag-and-drop fix path is: `GestureHandlerRootView` at root + `react-native-gesture-handler` + long-press-to-activate drag + axis-aware gesture arbitration (so horizontal scroll and vertical drag stop fighting).
- Layout direction: **responsive to aspect ratio, NOT forced orientation** — must stay correct on Samsung Fold and tablets. See `RESPONSIVE_LAYOUT_PLAN.md`.
