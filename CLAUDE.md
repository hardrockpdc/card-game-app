...
- **Memory Match (new single-player game) — built 2026-07-03.** `game/memory.js` (pure logic, 15 tests) + `screens/MemoryGameScreen.js`. Easy 4×3 / Medium 4×4 / Hard 4×6 boards, match identical cards, tiered win reward (`memory` sp 100), counts toward win/Well-Rounded achievements. Registered as the `MemoryGame` route + a 9th tile on the Choose Game grid (styled 🧠 placeholder — **real thumbnail art still TODO**). **How-to-Play entry still TODO** (that screen has elaborate per-game illustrated rules; not wired yet). Not yet device-tested.
