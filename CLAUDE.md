...
- **Memory Match (new single-player game) — built 2026-07-03.** `game/memory.js` (pure logic, 15 tests) + `screens/MemoryGameScreen.js` + `screens/MemoryDifficultyPickerScreen.js`. Easy 4×3 / Medium 4×4 / Hard 4×6, match identical cards, tiered win reward (`memory` sp 100), counts toward win/Well-Rounded achievements. Difficulty is chosen on the **picker screen** (`MemoryDifficultyPicker`, reached from the Choose Game tile) — the in-game screen has NO difficulty buttons (they were removed because a stray tap during play restarted the game). The Choose Game grid dropped its "Coming Soon" tile → **8 game tiles**, with Memory a styled 🧠 placeholder (**real thumbnail art still TODO**). **How-to-Play entry still TODO** (that screen has elaborate per-game illustrated rules; not wired yet). Not yet device-tested.
- **Coin economy: COMPLETE + code-reviewed.** Earn (tiered win rewards, daily-bonus streak, 15 achievements) and spend (decks/felts/frames) are all built and wired; ranks show on Profile. A high-effort review pass fixed a streak-counter bug (consecutive days reset every 7) and serialized the daily/achievement claim writes. 390 tests pass. **Not yet tested on a physical device** — that's the main next step.
...
- **⚠️ Firebase security rules — written but NOT deployed.** Hardened rules live in `database.rules.json` (only `rooms/*`; coins/profile/achievements are local). They must be pasted/deployed in the Firebase console (or `firebase deploy --only database`) **before public launch**, then online MP re-tested. See `POST_LAUNCH_CHECKLIST.md`.
