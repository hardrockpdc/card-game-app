...
export function runTests() {
  ...
  assert('family prompts are all tone=family', familyDeck.prompts.every((c) => c.tone === 'family'));
  assert('family answers are all tone=family', familyDeck.answers.every((c) => c.tone === 'family'));

  const matureDeck = createDeck('mature');
  assert('mature deck includes family prompts', matureDeck.prompts.some((c) => c.tone === 'family'));
  assert('mature deck includes mature prompts', matureDeck.prompts.some((c) => c.tone === 'mature'));
  assert('mature deck includes family answers', matureDeck.answers.some((c) => c.tone === 'family'));
  assert('mature deck includes mature answers', matureDeck.answers.some((c) => c.tone === 'mature'));
  assert('mature deck has more answers than family deck', matureDeck.answers.length > familyDeck.answers.length);
  ...
  const { players, answerDeck: remaining } = dealHands(rawPlayers, matureDeck.answers);
  assert('each player gets 10 cards', players.every((p) => p.hand.length === 10));
  assert('deck shrinks by 30', matureDeck.answers.length - remaining.length === 30);
  assert('scores initialised to 0', players.every((p) => p.score === 0));
  assert('no card dealt twice', (() => {
  ...
  assert('returns a prompt with text', !!prompt && typeof prompt.text === 'string');
  assert('deck shrinks by 1', matureDeck.prompts.length - deckAfterPick.length === 1);
  ...
  let blocked = false;
  try { processSubmission(state, 'host', players[0].hand[0].id); }
  catch (_) { blocked = true; }
  assert('judge cannot submit', blocked);

  let dupBlocked = false;
  try { processSubmission(state, '2', players[1].hand[1].id); }
  catch (_) { dupBlocked = true; }
  assert('duplicate submission blocked', dupBlocked);

  let badCardBlocked = false;
  try { processSubmission(state, '3', mariaCard.id); } // card not in Luis's hand
  catch (_) { badCardBlocked = true; }
  ...
  let badWinner = false;
  try { pickWinner(state, 'nonexistent-card-id'); }
  catch (_) { badWinner = true; }
  ...
  const atNineState  = { ...state, players: state.players.map((p) => p.id === '2' ? { ...p, score: 9  } : p) };
  const atTenState   = { ...state, players: state.players.map((p) => p.id === '2' ? { ...p, score: 10 } : p) };
  const aboveTenState = { ...state, players: state.players.map((p) => p.id === '2' ? { ...p, score: 11 } : p) };
