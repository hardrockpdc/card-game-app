...
**Update (animation work, Phase 4 — More games):** The initial-deal slide-in animation has been applied to four more games: Rummy, Conquián, Go Fish, and Poker (player's hole cards only). Each uses the same hasMountedRef + 100ms-per-card stagger pattern proven in Blackjack. Per-game initial hand sizes are different (Rummy 10, Conquián up to 10, Go Fish up to 7, Poker 2), and the stagger is only applied during the initial deal — single cards drawn mid-game animate alone with no delay. Out of scope for this phase: Last Card (uses a custom image map, not Card.js), Solitaire reveal flips (uses CardSlot indirection), opponents' hands in any multiplayer game, Poker community cards, Poker showdown reveals. Those remain instant pending future sessions.
